...
const palette={outer:[126,103,75],edge:[91,72,51],ring:[142,116,82],pillar:[157,128,91],cap:[168,138,97],cover:[110,84,56],low:[122,96,66],debris:[105,83,59]};
...
function drawSky(horizon){const sky=ctx.createLinearGradient(0,0,0,horizon);sky.addColorStop(0,'#6d9cb1');sky.addColorStop(.62,'#a9b8ad');sky.addColorStop(1,'#d0aa70');ctx.fillStyle=sky;ctx.fillRect(0,0,W,horizon);
const sunX=W*.72,sunY=horizon*.34;const sg=ctx.createRadialGradient(sunX,sunY,0,sunX,sunY,H*.18);sg.addColorStop(0,'rgba(255,231,170,.72)');sg.addColorStop(.15,'rgba(255,214,139,.20)');sg.addColorStop(1,'rgba(255,214,139,0)');ctx.fillStyle=sg;ctx.fillRect(0,0,W,horizon)}
function drawFloor(horizon){const g=ctx.createLinearGradient(0,horizon,0,H);g.addColorStop(0,'#9a774d');g.addColorStop(.38,'#745537');g.addColorStop(1,'#322419');ctx.fillStyle=g;ctx.fillRect(0,horizon,W,H-horizon);
ctx.save();ctx.globalAlpha=.18;ctx.strokeStyle='#e2c38e';for(let y=horizon+18;y<H;y+=Math.max(15,(y-horizon)*.22)){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(W,y);ctx.stroke()}ctx.globalAlpha=.10;for(let x=-W;x<W*2;x+=74){ctx.beginPath();ctx.moveTo(W/2+(x-W/2)*.12,horizon);ctx.lineTo(x,H);ctx.stroke()}ctx.restore()}
...
function render(){const horizon=H*(.49+pitch*.54-recoil*.12);drawSky(horizon);drawFloor(horizon);const rays=Math.max(320,Math.min(720,Math.floor(W/2.35))),fov=72*Math.PI/180,colW=W/rays;for(let i=0;i<rays;i++){const rel=(i/(rays-1)-.5)*fov,hit=cast(yaw+rel),dist=Math.max(.14,hit.t*Math.cos(rel));const wallH=Math.min(H*1.85,(H*.96)*(hit.h/3.2)/(dist*.071+.39));const y=horizon-wallH*.53;ctx.fillStyle=stoneColor(hit,dist,i);ctx.fillRect(i*colW,y,colW+1,wallH);
  ...
  if(dist<26){const alpha=Math.max(0,.16-dist*.0045);ctx.fillStyle=`rgba(48,35,23,${alpha})`;for(let k=1;k<5;k++)ctx.fillRect(i*colW,y+wallH*k/5,colW+1,1);if(((i+Math.floor(hit.hx*4))%17)===0)ctx.fillRect(i*colW,y,colW*.8,wallH)} }
// distance haze
